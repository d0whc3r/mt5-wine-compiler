...
![Docker](https://github.com/d0whc3r/mt5-wine-compiler/actions/workflows/docker-publish.yml/badge.svg)

A Docker image designed to compile MetaTrader 5 (MT5) `.mq5` files into `.ex5` executables using a headless Wine environment. This tool allows you to integrate MT5 compilation into your CI/CD pipelines or run it on Linux/macOS systems without needing a full Windows VM.
...
```bash
docker pull ghcr.io/d0whc3r/mt5-wine-compiler:master
```

Or build it locally:

```bash
git clone https://github.com/d0whc3r/mt5-wine-compiler.git
cd mt5-wine-compiler
docker build -t mt5-compiler .
```
...
To compile a single `.mq5` file, mount the directory containing your source code to `/home/wine/src` inside the container and pass the file path as an argument.

```bash
docker run --rm -v $(pwd)/src:/home/wine/src ghcr.io/d0whc3r/mt5-wine-compiler:master /home/wine/src/Expert.mq5
```

The compiled `Expert.ex5` file will be created in the same directory as the source file.
...
```bash
docker run --rm -v $(pwd)/src:/home/wine/src ghcr.io/d0whc3r/mt5-wine-compiler:master /home/wine/src/Expert.mq5 /home/wine/src/Compiled/Expert.ex5
```

### Using Custom Includes and Libraries

If your EA relies on custom include files (`.mqh`) or libraries (`.ex5`/`.dll`) that are not part of the standard MT5 distribution, you need to ensure they are available to the compiler.
...
**Command:**
```bash
docker run --rm -v $(pwd)/my-project:/home/wine/project ghcr.io/d0whc3r/mt5-wine-compiler:master /home/wine/project/Experts/MyExpert.mq5
```
...
# Find all .mq5 files in the Experts directory
find "$PROJECT_DIR/Experts" -name "*.mq5" | while read source_file; do
    ...
    docker run --rm \
        ...
        ghcr.io/d0whc3r/mt5-wine-compiler:master \
...
services:
  compiler:
    image: ghcr.io/d0whc3r/mt5-wine-compiler:master
    volumes:
      - ./src:/home/wine/src
    # Override command to compile a specific file
    command: ["/home/wine/src/Expert.mq5"]
```
...
You might see a warning like `WARNING: The requested image's platform (linux/amd64) does not match the detected host platform`. This is normal. The image forces `linux/amd64` because Wine and the MT5 compiler are 32-bit Windows applications that require x86 architecture. Docker's emulation handles this automatically.
