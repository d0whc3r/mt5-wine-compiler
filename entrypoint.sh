...
# Defaults
COMPILER_PATH="/home/wine/.mt5/drive_c/mt5/MetaEditor64.exe"
INCLUDE_PATH="C:\\mt5"
LOG_FILE="/home/wine/.mt5/drive_c/mt5/compilation.log"

# Check arguments
if [ "$#" -lt 1 ]; then
    echo "Usage: $0 <source_file_path> [output_path]"
    echo "Example: $0 src/Expert.mq5"
    exit 1
fi
...
INTERNAL_SOURCE_PATH="C:\\mt5\\$BASENAME"
cp "$SOURCE_FILE" "/home/wine/.mt5/drive_c/mt5/$BASENAME"
...
# /include:<path>
# /log[:<path>]
WINEPREFIX=/home/wine/.mt5 WINEARCH=win64 wine "$COMPILER_PATH" /compile:"$INTERNAL_SOURCE_PATH" /include:"$INCLUDE_PATH" /log:"$LOG_FILE" || true
...
if [ -f "$LOG_FILE" ]; then
    ...
    if echo "$LOG_CONTENT" | grep -q "0 errors"; then
        ...
        # If successful, the .ex5 file should be next to the source in drive_c
        # It will have the same basename but with .ex5 extension
        COMPILED_BASENAME="${BASENAME%.*}.ex5"
        COMPILED_FILE="/home/wine/.mt5/drive_c/mt5/$COMPILED_BASENAME"
        
        if [ -f "$COMPILED_FILE" ]; then
            # Determine destination
            if [ -z "$OUTPUT_PATH" ]; then
                # Default: replace .mq5 with .ex5 in the original path
                DESTINATION="${SOURCE_FILE%.mq5}.ex5"
            else
